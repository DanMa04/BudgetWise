...
import { createOverlay, removeOverlay } from "../src/content/overlay";
import type { CartCheckResponse } from "../src/shared/types";
...
describe("overlay", () => {
  ...
  it("creates overlay element in the DOM", () => {
    ...
    const host = document.getElementById("budgetwise-overlay-host");
  ...
  it("uses shadow DOM", () => {
    ...
    const host = document.getElementById("budgetwise-overlay-host");
    expect(host).not.toBeNull();
    // Shadow root is "closed" so we can't access it directly,
    // but the element itself should exist
  ...
  it("removes existing overlay before creating new one", () => {
    ...
    const hosts = document.querySelectorAll("#budgetwise-overlay-host");
    expect(hosts.length).toBe(1);
  });

  it("removeOverlay removes the host element", () => {
    createOverlay(makeResponse());
    expect(document.getElementById("budgetwise-overlay-host")).not.toBeNull();
    removeOverlay();
    expect(document.getElementById("budgetwise-overlay-host")).toBeNull();
  });
  ...
  it("auto-dismisses green warnings after timeout", () => {
    ...
    // The auto-dismiss has a 5s delay + 300ms animation
